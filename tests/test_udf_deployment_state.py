...
def test_history_rejects_a_different_aws_environment():
    first = _result("first-version")
    history = DeploymentHistory(name="policy-prod").append(first)

    with pytest.raises(ValueError, match="AWS account"):
        history.append(
            replace(
                _result("second-version"),
                aws_account_id="999999999999",
            )
        )


def test_schema_one_history_requires_fresh_environment_identity(tmp_path):
    path = tmp_path / "policy-prod.json"
    result = _result("first-version").to_dict()
    result.pop("aws_account_id")
    result.pop("aws_region")
    result.pop("stack_id")
    path.write_text(
        json.dumps(
            {
    ...
    latest = DeploymentStateStore(path, name="policy-prod").load().latest

    with pytest.raises(ValueError, match="cannot be changed safely"):
        latest.environment_identity()
