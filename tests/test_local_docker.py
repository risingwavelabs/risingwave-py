...
def test_docker_single_node_command(monkeypatch):
    ...
    assert "io.risingwave.python.local=true" in args
    assert any(value.startswith("io.risingwave.python.local.config=") for value in args)
...
def test_connect_returns_existing_client_and_configures_udf(client_type, monkeypatch):
    ...
    launcher.close()
    client.udf.close.assert_called_once_with()
...
def test_close_stops_container_before_local_udf_server(monkeypatch):
    launcher = DockerStandalone()
    events = []
    manager = MagicMock()
    manager.close.side_effect = lambda: events.append("udf")
    launcher._udf_managers.append(manager)
    launcher.started_here = True
    monkeypatch.setattr(launcher, "stop", lambda: events.append("container"))

    launcher.close()

    assert events == ["container", "udf"]
    assert launcher._udf_managers == []
...
def test_rejects_existing_container_with_different_configuration(monkeypatch):
    ...
    monkeypatch.setattr(
        launcher,
        "_existing_config",
        lambda: ("risingwavelabs/risingwave:v2.8.0", 5566, None),
    )
...
def test_rejects_unmanaged_existing_container_even_when_image_and_port_match(
    monkeypatch,
):
    launcher = DockerStandalone()
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        launcher,
        "_existing_config",
        lambda: (launcher.image, launcher.sql_port, None),
    )
...
def test_reuses_container_with_matching_managed_configuration(monkeypatch):
    launcher = DockerStandalone()
    monkeypatch.setattr(
        "risingwave.local.docker.shutil.which",
        lambda _: "/usr/bin/docker",
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            launcher.image,
            launcher.sql_port,
            launcher._managed_config_id(),
        ),
    )

    launcher.start()

    assert not launcher.started_here
...
def test_rejects_invalid_sql_port(port, error_type):
    with pytest.raises(error_type, match="sql_port"):
