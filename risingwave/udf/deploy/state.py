...
_SUPPORTED_STATE_SCHEMA_VERSIONS = (1, STATE_SCHEMA_VERSION)
        ...
        return cls(
            **{
                **value,
                ...
                "cpu_architecture": value.get("cpu_architecture", "X86_64"),
            }
        )
...
class DeploymentResult:
    ...
    def environment_identity(self) -> tuple[str, str, str]:
        """Return the AWS identity required to mutate this deployment safely."""

        if not self.aws_account_id or not self.aws_region or not self.stack_id:
            raise ValueError(
                f"deployment {self.deployment_id!r} predates AWS environment "
                "identity tracking and cannot be changed safely"
            )
        return self.aws_account_id, self.aws_region, self.stack_id
    ...
    def from_dict(cls, value: dict[str, Any]) -> "DeploymentResult":
        manifest = BundleManifest.from_dict(value["manifest"])
        return cls(
            **{
                **value,
                "aws_account_id": value.get("aws_account_id"),
                "aws_region": value.get("aws_region"),
...
class DeploymentHistory:
    ...
    def append(self, result: DeploymentResult) -> "DeploymentHistory":
        ...
        if self.latest is not None and (
            self.latest.environment_identity() != result.environment_identity()
        ):
            raise ValueError(
                "deployment AWS account, region, or stack ID does not match history"
            )
    ...
    def from_json(cls, text: str, *, expected_name: str) -> "DeploymentHistory":
        ...
        if value["schema_version"] not in _SUPPORTED_STATE_SCHEMA_VERSIONS:
            raise ValueError(
